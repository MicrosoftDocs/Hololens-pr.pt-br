...
Essa atualização agora mantém os dispositivos mais controlados pelo modo Quiosque, permitindo que ele volte a ter diferentes tipos de Quiosques antes de simplesmente apresentar um Quiosque vazio. Embora isso não seja gerenciável, isso pode ser algo para informar seu departamento de suporte se você estiver usando Quiosques de uma maneira que isso possa se aplicar à sua configuração.
...
- Em experiências WebXR, hologramas podem mudar ou inclinar quando você inclina a cabeça ou se move ao redor do ambiente.
- Dependendo da complexidade da experiência WebXR, a taxa de quadros pode soltar ou gaguejar.
- As juntas de mão articuladas ainda não estão disponíveis no WebXR.
...
- Você pode reproduzir vídeos 2D em um modo imersivo de "teatro", mas a taxa de quadros será inferior a 30 fps.
...
- A página Ethernet mostra um dispositivo Ethernet virtual ("UsbNcm") sempre (investigando). Esse dispositivo Ethernet virtual também será aparecer na página Rede da configuração do dispositivo, mas pode ser ignorado (investigando).
